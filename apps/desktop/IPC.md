...
- **UiSnapshot:** version, isDev, `devOnboardingStep` (dev builds only: open the onboarding wizard on step n for a capture, from `--dev-onboarding-step <n>`), profile (nickname, nation, userId), account (email, anonymous), pet (speciesId, stage, state), progress (localXp, serverXp, streakDays), hooks (status, mode, effectiveMode, probe), settings (scale, autostart), online (connected, lastSyncAt, lastError, configured), update status, notifications, battles (history, cooldownUntil, remainingToday, winStreak, loadout, unlockedMoveIds, treePoints, sharedPassivePoints, lastRespecAt), water (enabled, intervalMin, todayCount, nextDueAt).
