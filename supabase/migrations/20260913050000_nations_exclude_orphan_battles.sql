-- Nation battle tallies must only count battles whose challenger still exists (deleted test
-- accounts leave challenger_id null but keep their snapshot nation) and, as before, only count the
-- defender side for real players (Wild Mons have opponent_id null).
